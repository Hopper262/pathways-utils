...
sub Usage() { die "Usage: $0 <level_number> < <map.xml> > <level.obj>\n"; }
...
#       {
#         next unless exists $sref->{$card};
#         my ($tex, $short_ccw, $short_cw) = @{ $sref->{$card} };
#         my $wd = $walldirs{$card};
#         EdgeWall($row, $col, $colltype, $tex,
#             $short_cw  ? $wd->[3] : $wd->[2],
#             $short_ccw ? $wd->[1] : $wd->[0],
#             );
#       }
...
#     else
#     {
#       # build ceiling, render bits of floor as necessary
#       my @ceil = ();
#       for my $corner (qw(bl br tr tl))
#       {
#         my $cd = $cornerdirs{$corner};
